...
def conditional_ast_program():
    ...
    assign_output = lambda n: Assignment(Variable("OUT"), Expression([Literal(f"{n}")]))
    return Group(
        [
            Assignment(Variable("x"), Expression([Variable("IN")])),
            assign_output(-1),
...
def while_ast_program():
    ...
    return Group(
        [
            ...
            assign_literal("x", "1"),
            assign_literal("y", "0"),
            While(Expression([Variable("counter"), Literal("> 0")]), Group([
                assign_var("tmp", "x"),
                assign_var("x", "y"),
                Assignment(Variable("y"), Expression([Variable("y"), Literal("+"), Variable("tmp")])),
                Assignment(Variable("counter"), Expression([Variable("counter"), Literal("-1")])),
            ])),
            assign_var("OUT", "y"),
            Assignment(Variable("DONE"), Expression([Literal("0")]))
        ]
...
@pytest.mark.parametrize("program_inputs", [[0, 1, 9, 10, 15]])
...
def test_compile_from_ast(driver, program, program_inputs):
    ...
    for i in program_inputs:
        ...
        output, exit_code = run_program_js(
            driver=driver, desmos_js=js, program_input=i
        )
        assert exit_code == 0
